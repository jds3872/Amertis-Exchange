...
const TokensModal = ({
	...
	const currentToken = ToggleModal.forBase ? baseTokenData : quoteTokenData;

	// this saves the other current token
	const oppositeCurrentToken = ToggleModal.forBase
		? quoteTokenData
		: baseTokenData;
	...
	const handleTokenSelect = (arg: any) => {
		if (
			arg.ticker === currentToken.tokenName ||
			arg.ticker === oppositeCurrentToken.tokenName
		) {
			// console.log("SELECT ANOTHER TOKEN")
		} else {
